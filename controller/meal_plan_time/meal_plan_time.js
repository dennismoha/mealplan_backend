...
exports.getMealplanTime = async (req, res) => {
  ...
  try {
    ...
    if (mealplanTime.length === 0) {
      return res
        .status(404)
        .json({ message: "meal plan time resource not exisrs" });
    }
...
exports.createMealplanTime = async (req, res) => {
  ...
  const checkIfMealplanExistsSql =
    "SELECT * FROM mealplantime WHERE meal_plan_name = ?";
  const existingMealplan = await indexQuery.checkIfRecordExists(
    checkIfMealplanExistsSql,
    [mealPlanName]
  );
  ...
  const insertMealplanSql =
    "INSERT INTO mealplantime (meal_plan_name) VALUES (?)";
  ...
  res
    ...
    .send(
      getSuccessMessage(
        201,
        [],
        "succesfully created a new meal Plan time interval resource"
      )
    );
};
...
exports.updateMealplanTime = async (req, res) => {
  ...
  const updateMealplanSql =
    "UPDATE mealplantime SET meal_plan_name = ? WHERE  idmealPlanWeek = ?";

  await indexQuery.updateRecord(updateMealplanSql, [mealPlanName, id]);
  res
    .status(StatusCodes.OK)
    .send(getSuccessMessage(200, [], "succesfully updated"));
};

exports.deleteMealplanTime = async (req, res) => {
  let id = req.params.id;
  const sql = "DELETE FROM mealplantime WHERE  idmealPlanWeek  = ?";


    await indexQuery.deleteRecord(sql, [id]);
    res
    .status(StatusCodes.OK)
    .send(getSuccessMessage(200, [], "Deleted successfully"));  
