...
exports.handleRefreshToken = async (req, res) => {
  ...
  if (!user || user.status === 'revoked') return res.sendStatus(403);
  try {
    ...
    return res.json({ token, user: { id: user.idusers, email: user.email, role: user.role, status: user.status || 'active' } });
