...
exports.getCatalog = async (req, res) => {
  let [categories, subcategories, foodItems, mealTypes, mealSlots, links, recipes, countries, foodCountryLinks, mealCountryLinks, nutrition, recipeFoodLinks] = await Promise.all([
    prisma.foodcategory.findMany({ orderBy: { category_name: 'asc' } }), prisma.foodsubcategory.findMany({ orderBy: { subcategory_name: 'asc' } }), prisma.fooditems.findMany({ orderBy: { food_name: 'asc' } }), prisma.mealtype.findMany({ orderBy: { meal_name: 'asc' } }), prisma.meals.findMany({ select: { mealName: true, mealID: true }, orderBy: { mealName: 'asc' } }), prisma.mealmealtype.findMany({ include: { meals: true, mealtype: true } }), prisma.recipe.findMany({ orderBy: { title: 'asc' } }), prisma.countries.findMany({ orderBy: { name: 'asc' } }), prisma.food_item_countries.findMany(), prisma.meal_type_countries.findMany(), prisma.food_nutrition.findMany(), prisma.recipe_food_items.findMany(),
  ]);
