...
export function DiscoveryStrip({ catalog, onMeal, showExtras = true }: { catalog: Catalog; showExtras?: boolean; onMeal: (id: string, name: string) => void }) {
  return <><section className="discovery-section" id="meal-types"><div className="section-heading"><div><span className="eyebrow">From everyday staples to new favourites</span><h2>Explore meals</h2></div><a href="#library">See ingredients →</a></div><div className="discovery-rail">{catalog.mealTypes.map((meal, index) => <button type="button" style={{ textAlign: "left", cursor: "pointer" }} onClick={() => onMeal(meal.mealTypesID, meal.meal_name)} className={`meal-discovery tone-${index%5}`} key={meal.mealTypesID}><div className="meal-art"><span>{meal.meal_name.slice(0,1)}</span></div><div><small>{meal.countries?.[0]?.name || "Everyday cooking"}</small><h3>{meal.meal_name} →</h3><p>{catalog.recipes.some(recipe => recipe.meal_typeID === meal.mealTypesID) ? "View recipe" : "View meal · Recipe not added yet"}</p></div></button>)}</div></section>{showExtras && <section className="future-grid" id="budget"><article><span>Budget-friendly</span><h2>Eat well without overspending.</h2><p>Explore meal plans grouped by estimated weekly cost, from simple staples to generous family tables.</p><a href="#planner">Browse affordable plans →</a></article><article id="professionals"><span>Expert-led planning</span><h2>Plans made by professionals.</h2><p>Professional profiles, workplace details and their specialised meal-plan collections are coming next.</p><Link to="/register">Become a contributor →</Link></article></section>}</>
}
