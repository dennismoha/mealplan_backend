...
import { useCreateCategoryMutation, useCreateFoodItemMutation, useCreateSubcategoryMutation, useUpdateFoodItemMutation } from "../store/mealPlanApi"
...
export function AddCatalogModal({ kind, catalog, close, saved, fail, item }: { item?: FoodItem; kind: Exclude<AddKind, null>; catalog: Catalog; close: () => void; saved: () => void; fail: (m: string) => void }) {
  const [name, setName] = useState(item?.english_name || item?.food_name || ""); const [localNames, setLocalNames] = useState<FoodLocalName[]>(item?.local_names || []); const [description, setDescription] = useState(item?.descriptionl || ""); const [image, setImage] = useState(item?.image_url || ""); const [category, setCategory] = useState(item?.category_id || catalog.categories[0]?.food_categoryID || ""); const [subcategory, setSubcategory] = useState(item?.foodsubcategory_id || ""); const [busy, setBusy] = useState(false)
